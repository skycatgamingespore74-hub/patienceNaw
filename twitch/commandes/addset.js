...
module.exports = {
  ...
  execute(client, channel, tags, args) {
    ...
    if (!target) {
      client.say(channel, `⚠️ Usage: !addset <nom>`);
      return;
    }
    ...
    if (queue.includes(target)) {
      client.say(channel, `⚠️ ${target} est déjà dans la file d'attente.`);
      return;
    }
    ...
    client.say(channel, `✅ ${target} a été ajouté à la file d'attente.`);
    console.log(`[${new Date().toLocaleTimeString()}] 🔄 ${target} ajouté à la file d'attente par ${tags['display-name']}`);
  }
};
